...
| scrolling a coding agent's pane is slower than the pane beside it | ⚠ **the two panes are not doing the same thing, and the difference is measurable.** Ask the multiplexer: `tmux display-message -p -t <session> '#{alternate_on} #{mouse_any_flag} #{history_size}'`. A program on the ALTERNATE screen holding the mouse (`1 1`, and a history of almost nothing) owns its transcript — every scroll is a report it must read on its own render loop, and a busy one reads late. A program printing normally (`0 0`, a real history) is scrolled by the multiplexer itself, locally and instantly. Measured on one fleet: 58 lines behind the first kind, 987 behind the second. The dashboard paces itself to what the program can take and will not queue ahead of it, but it cannot make that program read sooner — `?gesture=1` shows the redraw wait per notch |
