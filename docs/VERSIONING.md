...
|---|---|---|---|
| **The repository (pdmux itself)** | `version` in the root `package.json` | `0.12.3` | every product release |
| **The agent** | `AgentVersion` in `agent/internal/cli/version.go` | `0.1.26` | only when `agent/**` or `packages/protocol/**` changes |
