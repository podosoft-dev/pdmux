...
Drift is caught in CI by `npm run schema:check -w @pdmux/protocol` / `expect:check`
(comparing generated bytes against committed bytes **byte for byte**) and by the Go-side
hash pin test. Which means **the output has to be deterministic** — keys sorted, a trailing
