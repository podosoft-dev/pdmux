...
bun run --cwd packages/protocol schema:build
...
bun run --cwd packages/protocol expect:build
...
# 8. verify both sides
go test ./...
cd .. && bun run --cwd packages/protocol test
```

Drift is caught by `bun run --cwd packages/protocol schema:check` / `expect:check` (comparing
