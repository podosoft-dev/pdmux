...
The published CLI drives this deployment: the `docker-compose` driver and `sync`
shipped in 0.17.0, so `npx @podosoft/podokit` below is the real invocation. (It was
not, until then — this project ran an unreleased build, and the workspace record kept
the local path. That is history now.)
