<script lang="ts">
	 ...
	 *
	 * Recursive by way of `<svelte:self>`: a tree is the one shape where the
	 * component and the data have the same recursion, and flattening it here would
	 * mean re-deriving depth on every row.
	 */
	...
	let { nodes, depth = 0, t }: Props = $props();
	...
	{#each nodes as node (node.path)}
		{#if node.kind === 'dir'}
			<li class="pdmux-tree-dir" data-pdmux-tree-dir={node.path}>
				...
				<FileTree nodes={node.children} depth={depth + 1} {t} />
			</li>
		{:else}
