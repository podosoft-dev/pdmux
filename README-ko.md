...
서로 다른 작업을 동시에 볼 수 있습니다. 헤더를 누르면 해당 터미널만 확대되고, 같은 헤더를 다시 누르면
원래 분할과 배치로 돌아옵니다. 본문 클릭은 입력 대상을 선택하며, 헤더를 끌어서 자리를 바꿀 수도 있습니다.

세션은 호스트의 멀티플렉서(tmux)에 살아 있습니다. 브라우저를 닫아도 AI CLI는 계속 돌고, 다시 열면 그
세션에 그대로 붙습니다. 분할 배치와 어느 pane이 어느 세션이었는지는 계정에 저장되므로 다른 컴퓨터에서
열어도 같은 화면입니다.

**파일 탐색기** — 빈 폴더를 포함한 폴더 트리를 업로드하고, 폴더나 여러 항목을 ZIP64로 다운로드합니다.
진행률과 충돌 처리(덮어쓰기·건너뛰기)를 제공하며 새로고침·데스크탑 재시작 후에도 재개할 수 있습니다.
브라우저 지원, 24시간 보존, 업그레이드 순서는 [폴더 전송 안내](docs/FILE-TRANSFERS.md)를 참고하세요.
...
Git 패널과 분리 창은 표시 중 5초마다 자동 갱신하며, 선택한 커밋과 열린 파일은 유지합니다.
**다시 읽기**는 즉시 수집을 요청하고, **원격 확인**은 객체를 내려받지 않고 현재 원격 참조를 읽습니다.
두 버튼은 새 결과를 기다리며, 1분 안에 도착하지 않으면 재시도 안내를 표시합니다.
백그라운드 탭이나 숨겨진 패널에서는 자동 조회를 멈춥니다.
...
**서비스와 외부 연결** — 호스트가 띄운 서비스(포트·프로브 방식·URL)를 등록하면 에이전트가 하트비트마다
프로브합니다. 카드의 서비스 바로가기는 호스트별 선택 기능이며 기본값은 꺼짐입니다. 루프백·LAN 주소는
대개 같은 네트워크에서만 유용하기 때문입니다. 다른 곳에서도 접근해야 하는 HTTP·HTTPS 서비스는
Cloudflare Tunnel로 공개할 수 있습니다. 기본값은 재사용 Cloudflare Access 정책으로 보호하며, 인터넷 완전
공개는 별도의 명시적 확인이 필요합니다. Raw TCP 포워딩은 포함하지 않습니다.
...
**호스트 공통 설정**에서 제한된 권한의 Cloudflare API 토큰을 연결한 뒤, 호스트 상세 화면의 서비스 메뉴에서
**외부 연결 설정**을 선택합니다. pdmux가 `서비스-호스트.기본-도메인` 형식의 호스트명을 제안하며 저장 전에
수정할 수 있습니다. 호스트마다 원격 관리 터널 하나를 만들고 그 호스트의 서비스들을 ingress 설정에
연결합니다.

호스트 에이전트는 공식 `cloudflared` 릴리즈를 자기 상태 디렉터리에 내려받아 릴리즈 digest를 검증하고,
되돌리기용 이전 실행 파일을 보존합니다. 시스템 패키지나 별도 서비스를 설치하지 않습니다. 터널 토큰은
PostgreSQL에 암호화해 저장하고 에이전트 설정으로만 보내며, argv 대신 환경 변수로 `cloudflared`에
전달합니다.

토큰 권한·정리 순서·실패 상태는
[Cloudflare 설정 및 복구 안내](docs/OPERATIONS.md#2-7-publishing-http-services-with-cloudflare-tunnel)를
참조하세요.
