...
![pdmux 대시보드 — 호스트 카드·분할 터미널·읽기전용 커밋 그래프](docs/media/dashboard.png)

<details>
<summary>터미널 열기와 저장소 전환 (애니메이션)</summary>

![호스트와 세션을 고르면 빈 pane 이 채워지고, 저장소를 바꾸면 커밋 그래프가 따라 바뀐다](docs/media/demo.gif)

</details>

> 위 플릿은 실제가 아니다. `tools/demo-agent.mjs` 가 에이전트 쪽 프로토콜을 말해서, 기계 없이도
> 대시보드를 보고 찍을 수 있게 한 것이다. 진짜 에이전트로 찍으면 그 기계에 있던 것이 그대로 실린다 —
> 남의 사용자명이 든 절대경로, 비공개 작업의 브랜치명, 내부망 서비스 URL.
...
위 스크린샷은 `tools/demo-agent.mjs` 로 찍었다. 에이전트 쪽 프로토콜을 말하므로 호스트가 하나도 없어도
대시보드가 채워진다:

```bash
# UI 에서 호스트를 만들고 상세 화면에서 토큰을 발급한 뒤
...
테스트 더블이 아니다 — 아무것도 이것에 대고 단언하지 않고, 두 구현을 같은 계약에 묶는 것은 적합성
코퍼스다. 제품을 눈으로 보기 위한 것이다.
